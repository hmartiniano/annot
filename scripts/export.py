...
def main(fname):
    ...
    for v in vcf:
        ...
        for consequence in v.INFO.get(annotation_key, "").split(","):
            ...
            if (csq["IMPACT"] == "HIGH"):
                keep = True
            elif ((csq["IMPACT"] == "MODERATE") and csq["SIFT"].startswith("deleterious(") and csq["PolyPhen"].startswith("probably_damaging") and (csq["CANONICAL"] == "YES")): 
                keep = True
            elif csq["CANONICAL"] == "YES":
                #keep = True
                pass
